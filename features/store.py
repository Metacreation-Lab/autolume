...
def _read(path):
    with np.load(path) as data:
        ...
        sigmas = np.array(data["sigmas"], dtype=np.float32)
    if metadata.get("version") != FORMAT_VERSION:
        return None
    return FeatureSet(directions=directions, sigmas=sigmas, metadata=metadata)
