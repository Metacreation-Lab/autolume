...
def load_network_pkl(f, force_fp16=False, custom=True, **ex_kwargs):
    ...
    if isinstance(data, tuple) and len(data) == 3 and all(isinstance(net, _TFNetworkStub) for net in data):
        ...
        nets = ['G_ema']
    # !!! custom
    if custom and not('synthesis.input.freqs' in data["G"].state_dict().keys()):  # new custom, only G
        data = create_networks(data, **ex_kwargs)
        nets = data.keys()
    else:  # new, full 3 netselse:
        nets = []
...
def convert_tf_generator(tf_G, custom=False, **ex_kwargs):
    ...
    if custom:
        ...
        from architectures.networks_stylegan2 import Generator
...
def convert_tf_discriminator(tf_D):
    ...
    # Convert params.
    from architectures import networks_stylegan2
