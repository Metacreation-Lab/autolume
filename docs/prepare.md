...
- Upscaling: Controls how images smaller than the target resolution are enlarged. The section shows how many of the imported images will be upscaled, and each of those images is marked with a small warning badge on its thumbnail.
  - Method: Standard (the default) uses classic resizing, which is fast and faithful to the source. AI Enhanced uses the Real ESRGAN model, which gives the best results on photographic sources. It can smooth fine textures such as canvas or film grain, and it takes noticeably longer. A large dataset can take minutes instead of seconds. The model weights are downloaded the first time you use it.
  - Denoise: Sets how much the AI upscaler smooths the image, from 0.0 to 1.0. The default is 0.0, which keeps grain and film texture. A value of 1.0 applies maximum smoothing. It only applies to AI Enhanced.
