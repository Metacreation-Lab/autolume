...
- Upscaling: Controls how images smaller than the target resolution are enlarged. Images below the target resolution are marked with a small warning badge on their thumbnail. Hover the badge to see the details.
  - AI Upscaling: Uses the Real ESRGAN model to enlarge images below the target resolution, instead of standard resizing. It is off by default. The model weights are downloaded the first time you use it. AI upscaling gives the best results on photographic sources. It can smooth fine textures such as canvas or film grain, and it takes noticeably longer. A large dataset can take minutes instead of seconds.
  - Denoise: Sets how much the AI upscaler smooths the image, from 0.0 to 1.0. The default is 0.0, which keeps grain and film texture. A value of 1.0 applies maximum smoothing.
