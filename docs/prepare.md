...
- Upscaling: Controls how images smaller than the target resolution are enlarged. The section shows how many of the imported images will be upscaled, and each of those images is marked with a small warning badge on its thumbnail.
  - Method: Basic (the default) uses classic resizing, which is fast and faithful to the source. AI Enhanced uses an AI upscaling model for sharper results and takes noticeably longer. A large dataset can take minutes instead of seconds. The model weights are downloaded when processing starts, so the first run needs an internet connection.
  - Model: Standard (4xNomosWebPhoto_RealPLKSR) is the default and suits most sources. Restore (4xRealWebPhoto_v4_dat2) is for low quality sources such as compressed JPEGs, noisy scans or small web images. It stays closest to the original and is several times slower.
