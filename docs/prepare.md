...
- Upscaling: Controls how images smaller than the target resolution are enlarged. The section shows how many of the imported images will be upscaled, and each of those images is marked with a small warning badge on its thumbnail.
  - Method: Basic (the default) uses classic resizing, which is fast and faithful to the source. AI Enhanced uses an AI upscaling model (4xNomosWebPhoto_RealPLKSR) to recover sharp detail, which makes small sources usable at the target resolution. It works best when your sources are of similar sizes and takes noticeably longer. A large dataset can take minutes instead of seconds. The model weights are downloaded when processing starts, so the first run needs an internet connection.
