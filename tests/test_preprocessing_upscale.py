...
def test_retired_model_key_falls_back_to_the_prepare_model(tmp_path, monkeypatch):
    seen = {}
    fake = Fake4x().eval()
    ...
    settings = _make_settings(tmp_path, image_size=64, target=128, ai_upscale=True)
    # Settings saved while Prepare still offered a second model.
    settings.upscaleSettings["model"] = "DAT2"
    messages = _run(settings)

    assert seen["model"] == upscale.PREPARE_MODEL
    assert any(m.get("type") == "completed" for m in messages if isinstance(m, dict))


def test_required_weights_for_the_prepare_model():
    assert upscale.required_weights(upscale.PREPARE_MODEL) == \
        [upscale.MODELS[upscale.PREPARE_MODEL]["weights"]]


def test_prepare_model_is_a_known_upscaler():
    assert upscale.PREPARE_MODEL in upscale.MODELS
