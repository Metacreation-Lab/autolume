...
# --- the ratio / noise predicate -----------------------------------------
#
# One function, two callers: `generator.noise_mode` maps the synthesis
# argument from a `RenderParams` and the Bending panel draws its note from a
# `ControlState`. Both shapes are asserted here, because a predicate that
# answered them differently would put the note on screen in exactly the
# frames the freeze it explains is not happening.
...
@pytest.mark.parametrize(
    ...
    [
        ({}, False),
        ({"noise_anim": True}, False),
        ({"noise_seed": 9}, False),
        ({"layer_ratios": (("L1", 2.0, 1.0),)}, False),
        ({"layer_ratios": (("L1", 2.0, 1.0),), "noise_anim": True}, True),
        ({"layer_ratios": (("L1", 1.0, 0.5),), "noise_seed": 9}, True),
        # A neutral pair stored explicitly, which the sparse rows can carry.
        ({"layer_ratios": (("L1", 1.0, 1.0),), "noise_anim": True}, False),
        ({"layer_ratios": (("L1", 1, 1),), "noise_anim": True}, False),
        # Noise off wins: nothing is asking for a random field at all.
        (
            {
                "layer_ratios": (("L1", 2.0, 1.0),),
                "noise_anim": True,
                "noise_enabled": False,
            },
            False,
...
def test_ratio_forces_const_noise_reads_both_state_shapes(changes, expected):
    state = params.ControlState(**changes)
    assert params.ratio_forces_const_noise(state) is expected
    assert (
        params.ratio_forces_const_noise(params.to_render_params(state)) is expected
    )


def test_ratio_forces_const_noise_finds_a_ratio_on_any_layer():
    """`noise_mode` is one global argument, so one layer decides the frame."""
    state = params.ControlState(
        layer_ratios=(("L1", 1.0, 1.0), ("L2", 1.0, 1.0), ("L3", 0.5, 1.0)),
        noise_anim=True,
    )
    assert params.ratio_forces_const_noise(state) is True
    assert params.ratio_forces_const_noise(params.to_render_params(state)) is True
